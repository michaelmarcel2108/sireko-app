// app/admin/rat/list-bulan/page.tsx
...
export default async function AdminRatListBulanPage() {
  ...
  const { data: ratList } = await supabase.from('rat_bulan').select('*').order('tahun', { ascending: false }).order('bulan', { ascending: false });

  return (
    <div className="space-y-6 p-4 md:p-8 max-w-7xl mx-auto">
      <h1 className="text-2xl font-extrabold tracking-tight text-slate-950 flex items-center gap-2">
        <ListTodo className="w-6 h-6 text-blue-600" /> List Arsip RAT Bulanan
      </h1>

      <Card className="shadow-sm border-slate-200 bg-white">
        <CardHeader className="p-6 border-b border-slate-100">
          <CardTitle className="text-lg font-bold text-slate-900">Rekapitulasi RAT Bulanan</CardTitle>
            ...
            <TableBody>
              {ratList && ratList.length > 0 ? ratList.map((item, index) => (
                <TableRow key={index}>
                  ...
                  <td className="font-extrabold text-sm text-slate-900 py-3.5 uppercase">{item.bulan || '-'} {item.tahun || ''}</td>
                ...
                <TableRow><TableCell colSpan={4} className="text-center py-12 text-slate-400">Belum ada arsip RAT Bulanan.</TableCell></TableRow>
              )}
            </TableBody>
          </Table>
        </CardContent>
