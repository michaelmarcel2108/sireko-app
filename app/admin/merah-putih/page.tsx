// app/admin/merah-putih/page.tsx
...
import { Flag, ChevronLeft, ChevronRight, MapPin } from 'lucide-react';
...
export default async function AdminMerahPutihPage({ searchParams }: PageProps) {
  ...
  // Catatan: Asumsi ada kolom penanda 'kategori_merah_putih' di tabel keragaan. 
  // Jika menggunakan tabel terpisah, ubah nama tabel di .from()
  let dbQuery = supabase
    .from('keragaan_koperasi') 
    .select('*', { count: 'exact' })
    // .eq('kategori_merah_putih', true) // <-- Aktifkan filter ini jika ada kolom khusus di database
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          <h1 className="text-2xl font-extrabold tracking-tight text-slate-950 flex items-center gap-2">
            <Flag className="w-6 h-6 text-red-600" /> Koperasi Merah Putih
          </h1>
          <p className="text-sm text-slate-500 font-medium">Monitoring daftar dan penyebaran Koperasi Merah Putih di Kabupaten Buleleng.</p>
        </div>
      </div>
            ...
            <Table>
              <TableHeader className="bg-slate-50/70">
                <TableRow className="border-b border-slate-100">
                  ...
                  <TableHead className="font-bold text-slate-500 text-center">Tahun Berdiri</TableHead>
                </TableRow>
              </TableHeader>
              <TableBody>
                {koperasiList && koperasiList.length > 0 ? (
                  koperasiList.map((item, index) => (
                    <TableRow key={item.id || index} className="hover:bg-slate-50/50 border-b border-slate-100">
                      ...
                      <td className="text-sm font-semibold text-slate-700 py-3.5">{item.kecamatan || '-'}</td>
                      <td className="text-sm font-medium text-slate-500 py-3.5 flex items-center gap-1 mt-1.5">
                         <MapPin className="w-3 h-3 text-slate-400" /> {item.desa || item.kelurahan || '-'}
                      </td>
                      <td className="text-sm font-bold text-slate-600 text-center py-3.5">{item.tahun_berdiri || item.tahun || '-'}</td>
                    </TableRow>
                  ))
                ) : (
                  <TableRow>
                    <TableCell colSpan={5} className="text-center py-12 text-slate-400 font-medium">Data koperasi tidak ditemukan.</TableCell>
                  </TableRow>
                )}
              </TableBody>
            </Table>
